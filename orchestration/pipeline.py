...
# Path to the sample repo that Architect Agent uses for file listing
SAMPLE_REPO_PATH = os.path.join(REPO_ROOT, "sample_repo")
...
# ------------------------------------------------------------------------------
# Agent implementations — PM and Architect use real modules; others are stubs
# ------------------------------------------------------------------------------

def call_pm_agent(task: dict) -> dict:
    """
    PM Agent: turns the feature_request into acceptance_criteria.
    Delegates to agents/pm_agent/pm_agent.py — real watsonx call when
    BOB_API_KEY is set, graceful error path (status=blocked) otherwise.
    """
    return run_pm_agent(task)


def call_architect_agent(task: dict) -> dict:
    """
    Architect Agent: scopes relevant files and writes an implementation plan.
    Delegates to agents/architect_agent/architect_agent.py — real watsonx
    call when BOB_API_KEY is set, graceful error path otherwise.
    Passes SAMPLE_REPO_PATH for disk-existence validation; falls back
    gracefully if the sample repo is not present.
    """
    repo = SAMPLE_REPO_PATH if os.path.isdir(SAMPLE_REPO_PATH) else ""
    return run_architect_agent(task, repo_path=repo)
